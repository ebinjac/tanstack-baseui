...
export const getApplicationGroups = createServerFn({ method: "GET" })
    ...
    .handler(async ({ data }) => {
        try {
            ...
            const groupedAppIds = new Set(membershipsWithApps.map((m) => m.applicationId));
            const ungroupedApplications = allApps.filter((a) => !groupedAppIds.has(a.id));
    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            const maxOrderResult = await db
                .select({ maxOrder: sql<number>`COALESCE(MAX(${applicationGroups.displayOrder}), 0)` })
...
export const addApplicationsToGroup = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data }) => {
        try {
            ...
            const maxOrderResult = await db
                .select({ maxOrder: sql<number>`COALESCE(MAX(${applicationGroupMemberships.displayOrder}), 0)` })
                .from(applicationGroupMemberships)
                .where(eq(applicationGroupMemberships.groupId, data.groupId));
            ...
            await db
                .delete(applicationGroupMemberships)
                .where(inArray(applicationGroupMemberships.applicationId, data.applicationIds));

            // Add new memberships
            const memberships = data.applicationIds.map((appId: string, index: number) => ({
        ...
        try {
            await db.transaction(async (tx) => {
                ...
                if (groupsToDelete.length > 0) {
                    await tx.delete(applicationGroups)
                        .where(inArray(applicationGroups.id, groupsToDelete.map(g => g.id)));
                }
                ...
                for (let i = 0; i < data.groups.length; i++) {
                    ...
                    if (!validUuidRegex.test(groupId)) {
                        const [newGroup] = await tx.insert(applicationGroups).values({
                            teamId: data.teamId,
                            name: groupData.name,
                            description: "",
                        ...
                        await tx.update(applicationGroups).set({
                            name: groupData.name,
                            displayOrder: i,
                            updatedAt: new Date(),
                        }).where(eq(applicationGroups.id, groupId));
                    ...
                    await tx.delete(applicationGroupMemberships)
