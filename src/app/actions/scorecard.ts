    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Failed to create scorecard entry'
...
export const updateScorecardEntry = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (newIdentifier && newIdentifier !== entry.scorecardIdentifier) {
      const existing = await db.query.scorecardEntries.findFirst({
        where: (entries, { eq }) =>
          eq(entries.scorecardIdentifier, newIdentifier),
      })
      if (existing) {
        throw new Error(
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Failed to update scorecard entry'
      throw new Error(message)
    }
  })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Failed to delete scorecard entry'
      throw new Error(message)
    }
  })
...
export const getGlobalScorecardData = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.leadershipFilter && data.leadershipType) {
      ...
      allApps = allApps.filter((app) => {
        switch (data.leadershipType) {
          case 'svp':
            return (
              app.ownerSvpName?.toLowerCase().includes(filterValue) ||
              app.ownerSvpEmail?.toLowerCase().includes(filterValue)
            )
          case 'vp':
            return (
              app.vpName?.toLowerCase().includes(filterValue) ||
              app.vpEmail?.toLowerCase().includes(filterValue)
            )
          case 'director':
            return (
              app.directorName?.toLowerCase().includes(filterValue) ||
              app.directorEmail?.toLowerCase().includes(filterValue)
            )
          case 'app_owner':
            return (
              app.applicationOwnerName?.toLowerCase().includes(filterValue) ||
              app.applicationOwnerEmail?.toLowerCase().includes(filterValue)
            )
          case 'app_manager':
            return (
              app.applicationManagerName?.toLowerCase().includes(filterValue) ||
              app.applicationManagerEmail?.toLowerCase().includes(filterValue)
            )
          case 'unit_cio':
            return (
              app.unitCioName?.toLowerCase().includes(filterValue) ||
          ...
          default:
            ...
            return [
              ...
              app.directorName,
              app.directorEmail,
              app.applicationOwnerName,
              app.applicationOwnerEmail,
              app.applicationManagerName,
              app.applicationManagerEmail,
              app.unitCioName,
              app.unitCioEmail,
    ...
    const leadershipOptions = {
      svp: [
        ...new Set(allApps.map((a) => a.ownerSvpName).filter(Boolean)),
      ].sort(),
      vp: [...new Set(allApps.map((a) => a.vpName).filter(Boolean))].sort(),
      director: [
        ...new Set(allApps.map((a) => a.directorName).filter(Boolean)),
      ].sort(),
      appOwner: [
        ...new Set(allApps.map((a) => a.applicationOwnerName).filter(Boolean)),
      ].sort(),
      unitCio: [
        ...new Set(allApps.map((a) => a.unitCioName).filter(Boolean)),
      ].sort(),
    }
...
export const publishScorecard = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to publish scorecard'
      throw new Error(message)
    }
  })
...
export const unpublishScorecard = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to unpublish scorecard'
      throw new Error(message)
    }
  })
