...
import { createLink, getLinkCategories, createLinkCategory, updateLink } from "@/app/actions/links"
...
export function CreateLinkDialog({
    ...
    const mutation = useMutation({
        mutationFn: (variables: { data: z.infer<typeof CreateLinkSchema> }) => createLink(variables),
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (variables: { data: { id: string } & Partial<z.infer<typeof CreateLinkSchema>> }) => updateLink(variables),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["links", teamId] })
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-y-auto p-0 gap-0 bg-background/95 backdrop-blur-xl border-border/50 shadow-2xl">
                ...
                <form onSubmit={form.handleSubmit(onSubmit as any, onError)} className="space-y-6 p-6">
                        ...
                        <div className="grid gap-2 relative">
                            ...
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    {...form.register("url")}
                                    disabled={mode === 'view'}
                                    className={cn("pl-9 bg-muted/30 border-muted-foreground/20 focus:bg-background transition-all", form.formState.errors.url && "border-destructive ring-destructive/20")}
                                />
                            </div>
                            {form.formState.errors.url && <span className="text-xs text-destructive ml-1">{form.formState.errors.url.message as string}</span>}
                        </div>
                        ...
                        <div className="grid gap-2">
                            ...
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    {...form.register("title")}
                                    disabled={mode === 'view'}
                                    className={cn("pl-9 bg-muted/30 border-muted-foreground/20 focus:bg-background transition-all", form.formState.errors.title && "border-destructive ring-destructive/20")}
                                />
                            </div>
                            {form.formState.errors.title && <span className="text-xs text-destructive ml-1">{form.formState.errors.title.message as string}</span>}
                        </div>
                    </div>
                                    ...
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[11px] text-muted-foreground">Only visible to you</span>
                                            </div>
                                        </Label>
                                    </div>
                                    <div className="relative">
                                        ...
                                        >
                                            <div className={cn("p-2 rounded-full border shadow-sm", field.value === "public" ? "bg-background" : "bg-background")}>
                                                <Globe2 className={cn("h-5 w-5", field.value === "public" ? "text-primary" : "text-muted-foreground")} />
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="text-sm font-semibold">Public</span>
                                                <span className="text-[11px] text-muted-foreground">Visible to everyone</span>
                                            </div>
                                        </Label>
                                    </div>
                                </RadioGroup>
                                        ...
                                        <SelectTrigger className="bg-muted/30 border-muted-foreground/20 w-full">
                                            <SelectValue>
                                                {field.value && field.value !== "none"
                                                    ? applications?.find((app) => app.id === field.value)?.applicationName || "Select Application"
                                                    : "Select Application"
                                                }
                                            </SelectValue>
                                        </SelectTrigger>
                        ...
                        <div className="grid gap-2">
                            ...
                            {isCreatingCategory ? (
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        {createCategoryMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                                    </Button>
                                    <Button
                                        type="button"
                                        variant="ghost"
                    ...
                    <div className="grid gap-2">
                        ...
                        <div className="bg-muted/30 border border-muted-foreground/20 rounded-md p-2 focus-within:ring-1 focus-within:ring-ring focus-within:border-ring transition-all">
                            <div className="flex flex-wrap gap-2 mb-2">
                                {(form.watch('tags') as string[])?.map((tag) => (
                                    <Badge key={tag} variant="secondary" className="gap-1 pr-1 pl-2 py-0.5 text-xs bg-background border shadow-sm">
                                        {tag}
                                        <button
                                            type="button"
                                ...
                                <input
                                    ...
                                    placeholder={mode === 'view' ? "" : (form.getValues('tags')?.length ? "" : "Type tag and press Enter...")}
                                    value={tagInput}
                        ...
                        <p className="text-[10px] text-muted-foreground ml-1">Press Enter or Comma to add a tag</p>
                    </div>
