...
type ParsedLink = z.infer<typeof CreateLinkSchema> & { id: string; isValid: boolean; error?: string }

export function UniversalImporterDialog({ teamId }: UniversalImporterDialogProps) {
    ...
    const handleImport = () => {
        ...
        const payload = validLinks.map(({ id, isValid, error, teamId: _, ...rest }) => rest)
...
"
                        ...
                        <div className="flex flex-col h-full gap-4">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <div className="flex gap-2">
                                    <Button variant="ghost" size="sm" onClick={() => setAllVisibility('public')} className="text-xs h-7">
                                        Set all Public
                                    </Button>
                                    <Button variant="ghost" size="sm" onClick={() => setAllVisibility('private')} className="text-xs h-7">
                                        Set all Private
                                    </Button>
                                </div>
                            </div>
                            ...
                            <ScrollArea className="flex-1 border rounded-md p-4">
                                <div className="space-y-4">
                                    {parsedLinks.map((link) => (
                                        <div key={link.id} className="flex gap-4 items-start p-3 bg-muted/30 rounded-lg group hover:bg-muted/50 transition-colors">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <div className="flex shrink-0">
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            className={cn("h-8 px-2", link.visibility === 'public' ? 'text-blue-500' : 'text-amber-500')}
                                                            onClick={() => updateLink(link.id, { visibility: link.visibility === 'public' ? 'private' : 'public' })}
                                                        >
                                                            {link.visibility === 'public' ? <Globe2 className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
                                                        </Button>
                                                        <Button variant="ghost" size="sm" className="h-8 w-8 text-destructive opacity-50 group-hover:opacity-100" onClick={() => removeLink(link.id)}>
                                                            <Trash2 className="w-4 h-4" />
                                                        </Button>
                                                    </div>
                                                </div>
                                                <div className="flex gap-2 items-center px-3">
                                                    <span className="text-xs text-muted-foreground font-mono truncate max-w-[500px]">{link.url}</span>
                                                </div>
                                                <Input
                ...
                <DialogFooter className="mt-4">
                    {step === 'review' && (
                        <Button variant="outline" onClick={() => setStep('input')}>Back to Paste</Button>
                    )}
                    ...
                    {step === 'input' ? (
                        ...
                        <Button onClick={handleImport} disabled={parsedLinks.length === 0 || mutation.isPending}>
