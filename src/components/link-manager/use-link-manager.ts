...
export function useLinkManager({
    ...
    const bulkUpdateMutation = useMutation({
        mutationFn: (params: { teamId: string; linkIds: string[]; updates: { visibility?: 'public' | 'private'; categoryId?: string | null } }) =>
    ...
    const selectAllLinks = useCallback(() => {
        setSelectedLinks(prev => prev.size === links.length ? new Set() : new Set(links.map(l => l.id)))
    }, [links])
