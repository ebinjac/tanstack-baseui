...
export function SessionGuard({ session, children }: SessionGuardProps) {
    ...
    const needsSessionInit = !session && ssoUser && sessionStatus !== 'error' && sessionStatus !== 'success'
    const isLoading = sessionStatus === 'loading' || needsSessionInit || waitingForSSO
                ...
                try {
                    ...
                    if (error instanceof Error) {
                        if (error.message.includes('Failed query') || error.message.includes('database')) {
                            setErrorMessage('Unable to connect to the Faze database. Please try again later or contact support if the issue persists.')
                        } else {
                            setErrorMessage(error.message || 'An unexpected error occurred while establishing your session.')
                        }
                    } else {
                        setErrorMessage('An unexpected error occurred while establishing your session.')
                    }
                }
            }
        }
            ...
            <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
                ...
                >
                    ...
                    <div className="space-y-3">
                        <h2 className="text-2xl font-bold tracking-tight">Initializing Session</h2>
        ...
        return (
            <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
                ...
                >
                    ...
                    <div className="space-y-3">
                        <h2 className="text-2xl font-bold tracking-tight text-red-500">Connection Failed</h2>
                    ...
                    <div className="w-full bg-card border border-border/50 rounded-2xl p-6 space-y-4">
                        <div className="flex items-center gap-3 text-left">
                            ...
                            <span className="text-sm font-medium">Database Service: <span className="text-red-500">Unavailable</span></span>
                        </div>
                        <div className="border-t border-border/50 pt-4">
                            <p className="text-xs text-muted-foreground">
                                The Faze database is currently unreachable. This is required to establish your session and load team permissions.
                            </p>
                        </div>
                    </div>
