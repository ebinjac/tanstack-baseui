...
export function OverviewTab({ team, stats, isAdmin, onEditTeam }: OverviewTabProps) {
    return (
        <div className="space-y-6">
            {/* Stats Row */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatsCard icon={<Boxes className="h-5 w-5 text-blue-500" />} label="Total Apps" value={stats.total} subLabel="Managed assets" />
                <StatsCard icon={<Activity className="h-5 w-5 text-green-500" />} label="Production" value={stats.active} subLabel="Live services" />
                <StatsCard icon={<ShieldAlert className="h-5 w-5 text-red-500" />} label="Critical (T0-T2)" value={stats.tiers.critical} subLabel="High availability" />
                <StatsCard icon={<Fingerprint className="h-5 w-5 text-purple-500" />} label="Status" value={team.isActive ? "Active" : "Inactive"} subLabel="Team state" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                    <CardHeader className="pb-4">
                        <div className="flex items-center justify-between">
                            ...
                            {isAdmin && (
                                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={onEditTeam}>
                ...
                <Card>
                    <CardHeader className="pb-4">
                        ...
                        <CardDescription>Team escalation lead and point of contact.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="flex items-center gap-4 p-4 rounded-lg border bg-muted/30">
                            ...
                            <div className="min-w-0 flex-1">
                                <p className="text-sm font-semibold truncate">{team.contactName || 'Not assigned'}</p>
                                {team.contactEmail && (
                                    <a
