...
function DroppableGroup({ group, onRemove, onColorChange }: DroppableGroupProps) {
    ...
    return (
        <div
            ...
            className={cn(
                ...
                isInvalid && !isOver && "border-amber-400/50 bg-amber-50/10 dark:bg-amber-950/10"
            )}
            style={{ borderLeftWidth: 4, borderLeftColor: isInvalid ? "#f59e0b" : group.color }}
        >
            {/* Group Header */}
            <div className={cn(
                ...
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className={cn(
                            "text-xs",
                            isInvalid ? "text-amber-600 dark:text-amber-400 font-medium" : "text-muted-foreground"
                        )}>
                            {isInvalid
                ...
                <div className="p-3 min-h-[100px] space-y-2">
                    {group.items.length === 0 ? (
                        <div className={cn(
                            ...
                            isOver ? "border-primary bg-primary/10 text-primary" : "border-amber-300 bg-amber-50/50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400"
                        )}>
                            {isOver ? "Drop here!" : (
                                <>
                                    <span className="font-medium">Drag 2+ apps here</span>
                                    <span className="text-[10px] opacity-70">to form a group</span>
                                </>
                            )}
                        </div>
                ...
                {items.length === 0 ? (
                    <div className={cn(
                        ...
                        {isOver ? (
                            ...
                            <>
                                <p className="text-sm font-medium">All applications are grouped!</p>
...
export function GroupManagementDragDrop({
    ...
    const saveMutation = useMutation({
        mutationFn: (data: {
            teamId: string;
            groups: { id: string; name: string; applicationIds: string[]; color?: string; }[];
        }) => syncGroupStructure({ data }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["application-groups", teamId] });
    ...
    const handleSave = () => {
        ...
        const invalidGroups = groups.filter(g => g.items.length < 2 && g.items.length > 0);

        if (invalidGroups.length > 0) {
            toast.warning(`${invalidGroups.length} group(s) with less than 2 applications will not be saved. A group needs at least 2 applications.`);
        }
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b bg-background shrink-0">
                <Button variant="outline" size="sm" onClick={handleCreateGroup} className="gap-2">
