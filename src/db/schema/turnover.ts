...
export const turnoverEntries = ensembleSchema.table(
    ...
    (table) => [
        ...
        index("turnover_entries_team_app_idx").on(table.teamId, table.applicationId),
...
export const finalizedTurnovers = ensembleSchema.table(
    ...
    (table) => [
        ...
        index("finalized_turnovers_team_date_idx").on(table.teamId, table.finalizedAt),
    ]
);
...
export const turnoverEntriesRelations = relations(turnoverEntries, ({ one }) => ({
    team: one(teams, {
...
export const turnoverRfcDetailsRelations = relations(turnoverRfcDetails, ({ one }) => ({
...
export const turnoverIncDetailsRelations = relations(turnoverIncDetails, ({ one }) => ({
    entry: one(turnoverEntries, {
...
export const turnoverMimDetailsRelations = relations(turnoverMimDetails, ({ one }) => ({
    entry: one(turnoverEntries, {
...
export const turnoverCommsDetailsRelations = relations(turnoverCommsDetails, ({ one }) => ({
    entry: one(turnoverEntries, {
...
export const finalizedTurnoversRelations = relations(finalizedTurnovers, ({ one }) => ({
    team: one(teams, {
...
export type TurnoverEntryWithDetails = TurnoverEntry & {
    application?: { applicationName: string; tla: string; tier?: string | null } | null;
