...
export function useDateRange(options: UseDateRangeOptions = {}): UseDateRangeReturn {
    ...
    // Validation helpers
    const canSetStart = useCallback((date: Date) => {
        if (minDate && date < minDate) return false
        if (maxDate && date > maxDate) return false
        return true
    }, [minDate, maxDate])

    const canSetEnd = useCallback((date: Date) => {
        if (minDate && date < minDate) return false
        if (maxDate && date > maxDate) return false
        return true
    }, [minDate, maxDate])
...
export function useYearSelection(options: UseYearSelectionOptions = {}): UseYearSelectionReturn {
...
export function useMonthYearSelection(
    ...
    const now = new Date()
    const { initialMonth = now.getMonth(), initialYear = now.getFullYear() } = options
