...
export function useScorecardMutations(teamId: string) {
    ...
    const invalidateYear = useCallback((year: number) => {
        return queryClient.invalidateQueries({
    ...
    const invalidateGlobal = useCallback((filters?: { year: number; leadershipType?: string; leadershipSearch?: string }) => {
        if (filters) {
            return queryClient.invalidateQueries({
...
export function useTeamMutations() {
    ...
    const invalidateDetail = useCallback((teamId: string) => {
        return queryClient.invalidateQueries({
            queryKey: teamKeys.detail(teamId),
        })
    }, [queryClient])

    const invalidateApplications = useCallback((teamId: string) => {
        return queryClient.invalidateQueries({
