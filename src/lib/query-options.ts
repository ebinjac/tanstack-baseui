...
import { scorecardKeys, turnoverKeys, linkKeys, teamKeys, adminKeys } from './query-keys'
import {
    getScorecardData,
    getPublishStatus,
 ...
 */
export const scorecardPublishStatusOptions = ({ teamId, year }: ScorecardDataOptions) =>
    queryOptions({
 ...
 */
export const turnoverEntriesOptions = (teamId: string, applicationIds?: string[], section?: string) =>
    queryOptions({
        queryKey: turnoverKeys.entries.section(teamId, applicationIds || [], section || ''),
        queryFn: () => getTurnoverEntries({ data: { teamId, applicationIds, section } }),
        staleTime: 1000 * 60 * 5, // 5 minutes
    })
 ...
 */
export const turnoverMetricsOptions = (teamId: string, startDate?: Date, endDate?: Date) =>
    queryOptions({
 ...
 */
export const finalizedTurnoversOptions = (teamId: string, page?: number, startDate?: Date, endDate?: Date, search?: string) =>
    queryOptions({
        queryKey: turnoverKeys.finalized.filtered(teamId, { from: startDate, to: endDate }, search, page),
        queryFn: () => getFinalizedTurnovers({ data: { teamId, page, startDate, endDate, search } }),
        staleTime: 1000 * 60 * 5, // 5 minutes
    })
