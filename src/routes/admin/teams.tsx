  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <MiniStatCard title="Active" value={stats.active} icon={CheckCircle2} color="emerald" />
        <MiniStatCard title="Inactive" value={stats.inactive} icon={XCircle} color="red" />
        <MiniStatCard title="With Admins" value={stats.withAdminGroup} icon={UserCheck} color="amber" />
      </div>
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow>
                    <TableCell colSpan={5} className="h-64 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <span className="text-muted-foreground text-sm">Fetching teams...</span>
                      </div>
                    </TableCell>
                  </TableRow>
                  ...
                  filteredTeams?.map((team) => (
                    <TableRow key={team.id} className="group hover:bg-muted/30 transition-colors">
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col gap-0.5">
                            ...
                            <div className="flex items-center gap-2">
                              <Badge variant="outline" className="text-[10px] h-4 font-normal bg-accent/50">U: {team.userGroup}</Badge>
                              <Badge variant="outline" className="text-[10px] h-4 font-normal bg-accent/50">A: {team.adminGroup}</Badge>
                            </div>
                          </div>
                        </div>
                      </TableCell>
                        ...
                        <div className="flex items-center gap-3">
                          <Avatar className="h-8 w-8 ring-1 ring-border shadow-sm">
                            <AvatarFallback className="bg-primary/5 text-primary text-xs font-bold">
                              {team.contactName.split(' ').map(n => n[0]).join('').toUpperCase()}
                            </AvatarFallback>
                          </Avatar>
                          <div className="flex flex-col">
                            <span className="font-medium leading-none">{team.contactName}</span>
                            <span className="text-xs text-muted-foreground mt-1">{team.contactEmail}</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(team.createdAt), { addSuffix: true })}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                            ...
                            <Tooltip>
                              <TooltipTrigger render={
                                <Button
                                  ...
                                  onClick={() => { setSelectedTeam(team); setIsDetailsOpen(true); }}
                                >
          ...
          <DialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <DialogDescription>Overview and configuration for this team.</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          {selectedTeam && (
            <div className="grid gap-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Ads User Group</span>
                  <p className="text-sm font-mono bg-muted/50 px-2 py-0.5 rounded border border-border w-fit">{selectedTeam.userGroup}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Ads Admin Group</span>
                  <p className="text-sm font-mono bg-muted/50 px-2 py-0.5 rounded border border-border w-fit">{selectedTeam.adminGroup}</p>
                </div>
              </div>
              <div className="space-y-1">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Primary Contact</span>
                <div className="p-3 rounded-lg border border-border bg-muted/20 flex flex-col gap-1">
                  <span className="text-sm font-bold">{selectedTeam.contactName}</span>
                  <span className="text-xs text-muted-foreground">{selectedTeam.contactEmail}</span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Created</span>
                  <p className="text-sm font-medium">{new Date(selectedTeam.createdAt).toLocaleDateString()}</p>
                  <p className="text-[10px] text-muted-foreground italic">{formatDistanceToNow(new Date(selectedTeam.createdAt), { addSuffix: true })}</p>
                </div>
                {selectedTeam.updatedAt && (
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium">{new Date(selectedTeam.updatedAt).toLocaleDateString()}</p>
                    <p className="text-[10px] text-muted-foreground italic">{formatDistanceToNow(new Date(selectedTeam.updatedAt), { addSuffix: true })}</p>
                  </div>
                )}
              </div>
              <div className="space-y-1">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Internal Identifiers</span>
                <p className="text-[10px] font-mono text-muted-foreground break-all bg-muted/30 p-2 rounded">ID: {selectedTeam.id}</p>
              </div>
            </div>
          )}
          <DialogFooter>
...
function StatusBadge({ active }: { active: boolean }) {
  if (active) {
    return <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-200/50 dark:border-emerald-800/50 gap-1.5 flex w-fit items-center"><CheckCircle2 className="h-3 w-3" /> Active</Badge>
  }
  return <Badge className="bg-red-100 text-red-700 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-400 border-red-200/50 dark:border-red-800/50 gap-1.5 flex w-fit items-center"><XCircle className="h-3 w-3" /> Inactive</Badge>
}
