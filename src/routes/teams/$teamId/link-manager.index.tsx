...
// import { Tabs, TabsList, TabsTrigger } from '@/components/ui/tabs' // Unused
      ...
      <div className="max-w-7xl mx-auto space-y-8 p-8 pt-6">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="relative overflow-hidden transition-all duration-300 border border-border/50 bg-card/40 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 group hover:border-primary/30">
              <div className="absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 opacity-[0.03] rounded-full bg-primary" />
              <div className="h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-primary/10 text-primary border border-primary/20 relative z-10 group-hover:scale-110 transition-transform">
                <Grip className="h-5 w-5" />
              </div>
              <div className="relative z-10 min-w-0 flex flex-col justify-center">
                <p className="text-2xl font-bold tabular-nums tracking-tighter leading-none">{stats.total}</p>
                <p className="text-[10px] text-muted-foreground font-bold mt-1.5 opacity-60">Total Resources</p>
              </div>
            </div>

            <div className="relative overflow-hidden transition-all duration-300 border border-border/50 bg-card/40 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 group hover:border-blue-500/30">
              <div className="absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 opacity-[0.03] rounded-full bg-blue-500" />
              <div className="h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-blue-500/10 text-blue-600 border border-blue-500/20 relative z-10 group-hover:scale-110 transition-transform">
                <Globe2 className="h-5 w-5" />
              </div>
              <div className="relative z-10 min-w-0 flex flex-col justify-center">
                <p className="text-2xl font-bold tabular-nums tracking-tighter leading-none text-blue-600">{stats.public}</p>
                <p className="text-[10px] text-muted-foreground font-bold mt-1.5 opacity-60">Public Access</p>
              </div>
            </div>

            <div className="relative overflow-hidden transition-all duration-300 border border-border/50 bg-card/40 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 group hover:border-amber-500/30">
              <div className="absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 opacity-[0.03] rounded-full bg-amber-500" />
              <div className="h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-amber-500/10 text-amber-600 border border-amber-500/20 relative z-10 group-hover:scale-110 transition-transform">
                <Lock className="h-5 w-5" />
              </div>
              <div className="relative z-10 min-w-0 flex flex-col justify-center">
                <p className="text-2xl font-bold tabular-nums tracking-tighter leading-none text-amber-600">{stats.private}</p>
                <p className="text-[10px] text-muted-foreground font-bold mt-1.5 opacity-60">Team Restricted</p>
              </div>
            </div>

            <div className="relative overflow-hidden transition-all duration-300 border border-border/50 bg-card/40 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 group hover:border-indigo-500/30">
              <div className="absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 opacity-[0.03] rounded-full bg-indigo-500" />
              <div className="h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-indigo-500/10 text-indigo-600 border border-indigo-500/20 relative z-10 group-hover:scale-110 transition-transform">
                <Activity className="h-5 w-5" />
              </div>
              <div className="relative z-10 min-w-0 flex flex-col justify-center">
                <p className="text-2xl font-bold tabular-nums tracking-tighter leading-none text-indigo-600">{stats.usage > 1000 ? `${(stats.usage / 1000).toFixed(1)}k` : stats.usage}</p>
                <p className="text-[10px] text-muted-foreground font-bold mt-1.5 opacity-60">Total Insights</p>
              </div>
            </div>
          </div>
        </div>

        {/* Bulk Actions Layer (Conditional) */}
        <AnimatePresence>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10 text-primary-foreground">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <button onClick={clearSelection} className="text-xs font-bold hover:underline opacity-70 hover:opacity-100 flex items-center gap-1">
                          <X className="h-3 w-3" /> Clear Selection
                        </button>
                        <span className="opacity-30">•</span>
                        <button onClick={selectAllLinks} className="text-xs font-bold hover:underline opacity-70 hover:opacity-100">
                          Select All Page
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center bg-white/10 backdrop-blur-md rounded-xl p-1 border border-white/10">
                      <Button onClick={() => handleBulkVisibility('public')} variant="ghost" size="sm" className="h-9 rounded-lg hover:bg-white/20 text-white font-bold text-xs gap-2">
                        <Globe2 className="h-3.5 w-3.5" /> Public
                      </Button>
                      <Button onClick={() => handleBulkVisibility('private')} variant="ghost" size="sm" className="h-9 rounded-lg hover:bg-white/20 text-white font-bold text-xs gap-2">
                        <Lock className="h-3.5 w-3.5" /> Private
                      </Button>
                    </div>

                    <Select onValueChange={(val) => handleBulkCategory(val === 'none' ? null : (val as string))}>
                      ...
                      <PopoverContent className="w-80 p-6 rounded-3xl" align="end">
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              placeholder="operations, secure, docs..."
                              value={bulkTagsInput}
                              onChange={(e) => setBulkTagsInput(e.target.value)}
                              className="h-11 rounded-xl bg-muted/50 border-transparent focus:bg-background"
                            />
                          </div>
                          <Button onClick={handleBulkAddTags} className="w-full h-11 rounded-xl font-bold text-xs" disabled={!bulkTagsInput.trim() || bulkUpdateMutation.isPending}>
                            {bulkUpdateMutation.isPending ? "Updating..." : "Add Tags"}
                          </Button>
                        </div>
                      </PopoverContent>
                    </Popover>
            ...
            <div className="flex items-center gap-3 flex-wrap xl:flex-nowrap">
              ...
              <div className="bg-muted/40 p-1.5 rounded-xl flex items-center gap-1 border border-border/50 h-12 px-1.5 shrink-0 relative">
                {[
                  ...
                  const isActive = (searchParams.visibility || 'all') === v.id
                  return (
                    <button
                      key={v.id}
                      onClick={() => handleVisibilityChange(v.id)}
                      className={cn(
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative h-9 w-9 rounded-lg flex items-center justify-center transition-colors z-10",
                        isActive ? "text-primary" : "text-muted-foreground hover:text-foreground"
                      )}
                    >
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-border/30 px-2 mt-2">
              ...
              {searchParams.search && (
                <Badge variant="secondary" className="h-6 gap-1 pr-1 pl-2 rounded-lg text-[11px] font-bold bg-background/50 border border-primary/20 shadow-sm">
                  "{searchParams.search}"
                  <button onClick={() => handleSearch("")} className="ml-1 hover:bg-primary/10 rounded-full p-0.5"><X className="h-3 w-3" /></button>
                </Badge>
              )}
              {selectedApp && (
                <Badge variant="secondary" className="h-6 gap-1 pr-1 pl-2 rounded-lg text-[11px] font-bold bg-blue-500/5 text-blue-600 border border-blue-500/20 shadow-sm">
                  ...
                  <button onClick={() => handleApplicationChange("all")} className="ml-1 hover:bg-blue-500/10 rounded-full p-0.5"><X className="h-3 w-3" /></button>
                </Badge>
              )}
              {selectedCategory && (
                <Badge variant="secondary" className="h-6 gap-1 pr-1 pl-2 rounded-lg text-[11px] font-bold bg-purple-500/5 text-purple-600 border border-purple-500/20 shadow-sm">
                  ...
                  <button onClick={() => handleCategoryChange("all")} className="ml-1 hover:bg-purple-500/10 rounded-full p-0.5"><X className="h-3 w-3" /></button>
                </Badge>
              )}
              {searchParams.visibility && searchParams.visibility !== 'all' && (
                <Badge variant="secondary" className="h-6 gap-1 pr-1 pl-2 rounded-lg text-[11px] font-bold bg-background/50 border border-border/20 shadow-sm capitalize">
                  ...
                  <button onClick={() => handleVisibilityChange("all")} className="ml-1 hover:bg-muted-foreground/10 rounded-full p-0.5"><X className="h-3 w-3" /></button>
                </Badge>
              )}
              <button onClick={clearAllFilters} className="text-xs font-bold text-muted-foreground hover:text-destructive flex items-center gap-1 ml-auto">
        ...
        <div className="relative">
          {links.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 bg-card/10 backdrop-blur-sm border border-dashed border-border/50 rounded-[2.5rem] text-center space-y-6">
              ...
              <Button variant="outline" onClick={clearAllFilters} className="h-11 px-8 rounded-2xl font-bold text-xs border-primary/20 hover:bg-primary/5">
                Clear Filters
              </Button>
            </div>
          ) : (
