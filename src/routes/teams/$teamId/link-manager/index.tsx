...
import { Search, Grip, Table as TableIcon, LayoutList, Globe2, Lock, Layers, Box, X, Tag, Activity, RotateCcw, Loader2, Plus } from 'lucide-react'
...
import { StatsSummaryItem, LinkManagerPage as PageWrapper, SubPageHeader } from '@/components/link-manager/shared'
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10 text-primary-foreground">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center bg-white/10 backdrop-blur-md rounded-xl p-1 border border-white/10">
              <Button onClick={() => onBulkVisibility('public')} variant="ghost" size="sm" className="h-8 text-white hover:bg-white/20 hover:text-white">
                <Globe2 className="h-3.5 w-3.5 mr-2" /> Make Public
              </Button>
              <Button onClick={() => onBulkVisibility('private')} variant="ghost" size="sm" className="h-8 text-white hover:bg-white/20 hover:text-white">
                <Lock className="h-3.5 w-3.5 mr-2" /> Make Private
              </Button>
            </div>

            <Select onValueChange={(val: string | null) => onBulkCategory(val === 'none' ? null : val)}>
            ...
            {isPending && <Loader2 className="h-5 w-5 animate-spin text-white/80" />}
          </div>
        </div>
      </div>
    </motion.div>
...
// ============================================================================
const ViewModeToggle = memo(function ViewModeToggle({ value, onChange }: { value: 'grid' | 'table' | 'compact'; onChange: (mode: any) => void }) {
  const options = [
      ...
      {options.map((option) => (
        <button
          ...
          className={cn(
            ...
            value === option.id ? "bg-background shadow-sm text-primary ring-1 ring-border/10" : "text-muted-foreground hover:bg-background/50 hover:text-foreground"
          )}
        >
...
const ActiveFiltersRow = memo(function ActiveFiltersRow({ searchParams, selectedApp, selectedCategory, onSearch, onApplicationChange, onCategoryChange, onVisibilityChange, onClearAllFilters }: any) {
  return (
    <div className="flex items-center gap-2 px-1 overflow-x-auto pb-1 scrollbar-hide">
      <span className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-wider shrink-0 mr-1">Active Filters:</span>
      {searchParams.search && (
        <Badge variant="secondary" className="h-6 gap-1 pr-1 pl-2 rounded-lg text-[11px] font-bold bg-primary/5 text-primary border border-primary/20 shadow-sm">
          "{searchParams.search}"
